...
Route::group(['middleware' => ['PreventBackHistory', 'guest']], function () {
    ...
    Route::post('login', [LoginController::class, 'submitLogin'])->middleware('throttle:login')->name('login.submit');
...
Route::get('logout', [LoginController::class, 'logout'])->name('auth.logout')->middleware(['auth', 'PreventBackHistory', 'RestrictIP']);